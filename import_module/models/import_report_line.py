...
class PentaImportReportLine(models.Model):
    ...
    state = fields.Selection(
        related="purchase_line_id.order_id.id_import.state",
        string="Estado",
        store=False
    )
    ...
    x_unit_import_cost = fields.Float(
        string="Costo unitario",
        digits=(16, 6),
        compute="_compute_unit_import_cost",
        store=True
    )
    x_import_factor = fields.Float(
        string="Factor de Importación (%)",
        digits=(16, 2),
        compute="_compute_import_factor",
        store=True
    )

    @api.depends("x_unit_import_cost", "x_price_unit")
    def _compute_import_factor(self):
        for rec in self:
            if rec.x_price_unit:
                rec.x_import_factor = (1 - (rec.x_unit_import_cost / rec.x_price_unit)) * 100
            else:
                rec.x_import_factor = 0.0

    @api.depends("x_total_importacion", "x_qty")
    def _compute_unit_import_cost(self):
        for rec in self:
            if rec.x_qty:
                rec.x_unit_import_cost = rec.x_total_importacion / rec.x_qty
            else:
                rec.x_unit_import_cost = 0.0
