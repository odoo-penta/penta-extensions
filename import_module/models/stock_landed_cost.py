...
class stock_landed_cost_module(models.Model):
    ...
    show_customs_taxes_tab = fields.Boolean(
        compute="_compute_show_customs_taxes_tab"
    )

    @api.depends_context("company")
    def _compute_show_customs_taxes_tab(self):
        param = self.env["ir.config_parameter"].sudo().get_param(
            "penta.calculate_customs_taxes"
        )
        value = param == "True"

        for rec in self:
            rec.show_customs_taxes_tab = value
    ...
    def create(self, vals):
        cost = super().create(vals)

        if cost.source_move_line_ids:
            for cost_line in cost.cost_lines:
                move_line = cost.source_move_line_ids.filtered(
                    lambda ml: ml.product_id == cost_line.product_id
                )[:1]

                if move_line:
                    cost_line.account_move_id = move_line.move_id.id
